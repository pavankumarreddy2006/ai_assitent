...
def _from_gnews(query: str) -> List[Dict]:
    ...
    try:
        ...
        r = requests.get(url, params={"q": query, "lang": "en", "max": 8, "apikey": GNEWS_API}, timeout=12)
        if r.status_code != 200:
            return []
        return [{"title": a.get("title", ""), "source": a.get("source", {}).get("name", "GNews")}
...
def _from_newsdata(query: str) -> List[Dict]:
    ...
    try:
        ...
        if r.status_code != 200:
            return []
        return [{"title": a.get("title", ""), "source": a.get("source_id", "NewsData")}
...
def _from_newsapi(query: str) -> List[Dict]:
    ...
    try:
        ...
        r = requests.get(url, params={"q": query, "sortBy": "publishedAt", "pageSize": 8, "apiKey": NEWS_API_KEY}, timeout=12)
        if r.status_code != 200:
            return []
        return [{"title": a.get("title", ""), "source": a.get("source", {}).get("name", "NewsAPI")}
                for a in r.json().get("articles", []) if a.get("title") and "[Removed]" not in a.get("title", "")]
    except Exception:
        return []
...
def fetch_news(query: str = "latest india news today") -> Tuple[List[Dict], str]:
    ...
    if any(k in q.lower() for k in ["kakinada", "andhra", "telugu", "vizag"]):
        search_q = q
    elif any(k in q.lower() for k in ["education", "college", "university", "school"]):
        search_q = f"{q} india education"
    else:
        search_q = q
