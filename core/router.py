...
def api_chat():
    try:
        ...
        if intent == "weather":
            reply = purify_reply(get_weather_from_query(user_message, lang=lang), max_lines=2, max_chars=220)
            return jsonify({
                "reply": reply,
        ...
        if intent == "news":
            ...
            reply = purify_reply(summarize_news(articles, lang=lang), max_lines=3, max_chars=320)
            return jsonify({
                "reply": reply,
        ...
        if intent == "search":
            reply = purify_reply(search_and_format(user_message, lang=lang), max_lines=3, max_chars=320)
            return jsonify({
                "reply": reply,
                "source": "Web Search",
                "show_images": False,
        ...
        return jsonify({
            "reply": purify_reply(ai_reply, max_lines=4, max_chars=360),
            "source": "AI Assistant",
            "show_images": False,
            "images": [],
            "show_video": False,
